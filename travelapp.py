...
def generate_itinerary(location_name, duration, companion, mood, budget, residence, free_request):
    try:
        system_prompt = (
            "あなたはプロの優れた旅行プランナーです。"
        ...
        user_request = (f"旅行先は「{location_name}」で、「{duration}」の旅行プランを提案してください。"
                       f"旅行の気分は「{mood}」、予算は「{budget}」、居住地は「{residence}」です。これらの条件も考慮してください。"
        )
        if free_request:
            user_request += f"\nまた、以下の特別な要望があります。「{free_request}」。この要望も考慮してください。"
    ...
    if st.button("提案を生成", type="primary"):
        if not selected_residence:
            ...
            with st.spinner("提案を生成中..."):
                suggestion_text= generate_travel_suggestion(
                    selected_mood,
                    select_companion,
                    location_type,
                    selected_budget,
        ...
        if suggestion.strip() and "【場所】" in suggestion:
            ...
            if match:
                ...
                if st.button(f"「{location_name}」の詳細プランを見る", key=f"plan_button_{i}"):
                    with st.spinner(f"「{location_name}」の詳細プランを生成中..."):

                        user_inputs = st.session_state.user_inputs
                        st.session_state.itinerary = generate_itinerary(
                            location_name,
                            user_inputs["duration"],
                            user_inputs["companion"],
                            user_inputs["mood"],
                            user_inputs["budget"],
                            user_inputs["residence"],
                            user_inputs["free_request"]
                        )

if st.session_state.itinerary:
    st.markdown("### 詳細な旅行プラン")
    st.info(st.session_state.itinerary)
    if st.button("プランを隠す"):
        st.session_state.itinerary = None
        st.rerun()
